...
# Step 3: Convert the first 8 bytes (16 hex digits) of the SHA-1 hash to decimal
# Extract the first 8 bytes (16 hex characters)
...
echo "        n: bigInt('$(openssl rsa -in "$PRIVATE_KEY_FILE" -pubout -text | grep 'Modulus' | awk '{print $2}')'),"
echo "        e: 65537,"
echo "    },"
echo "]"
