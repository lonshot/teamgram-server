...
export REDIS_HOST=${REDIS_HOST:-"redis:6370"}
export KAFKA_HOST=${KAFKA_HOST:-"kafka:9092"}
export MYSQL_URI=${MYSQL_URI:-"teamgram:teamgram@tcp(mysql:3360)/teamgram?charset=utf8mb4"}
...
createConfigs() {
  ...
  for file in `ls $CONFIG_TEMPLATES_DIR`; do
    cat $CONFIG_TEMPLATES_DIR/$file \
      ...
      | sed "s#127.0.0.1:6370#$REDIS_HOST#g" \
      | sed "s#localhost:6370#$REDIS_HOST#g" \
      | sed "s#root:@tcp(127.0.0.1:3360)/teamgram?charset=utf8mb4#$MYSQL_URI#g" \
