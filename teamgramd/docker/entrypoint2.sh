...
export MYSQL_URI=${MYSQL_URI:-"teamgram:teamgram@tcp(mysql:4406)/teamgram?charset=utf8mb4"}
...
createConfigs() {
  ...
  for file in `ls $CONFIG_TEMPLATES_DIR`; do
    cat $CONFIG_TEMPLATES_DIR/$file \
      ...
      | sed "s#root:@tcp(127.0.0.1:4406)/teamgram?charset=utf8mb4#$MYSQL_URI#g" \
