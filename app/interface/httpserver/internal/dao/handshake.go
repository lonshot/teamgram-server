...
func (m *HandshakeStateCtx) DebugString() (s string) {
...
func (d *Dao) GetHandshakeStateCtx(nonce []byte) *HandshakeStateCtx {
	ctx, _ := d.handshakeStateCtxCache.Get(hex.EncodeToString(nonce))
	return ctx.(*HandshakeStateCtx)
}

func (d *Dao) PutHandshakeStateCtx(state *HandshakeStateCtx) {
	d.handshakeStateCtxCache.Set(hex.EncodeToString(state.Nonce), state)
}
