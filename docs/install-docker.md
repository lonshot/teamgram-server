...
RUN mkdir -p /teamgram
COPY --from=build /teamgram/pwm /teamgram/pwm

WORKDIR /teamgram/pwm/etc
RUN rm -rf *

WORKDIR /teamgram/pwm/bin
RUN chmod -R 777 *.sh
...
- teamgram-net
volumes:
- ./docker/etc:/teamgram/pwm/etc
- /Volumes/data/teamgram/volumes/logs:/teamgram/pwm/logs
depends_on:
- mysql
- redis
- minio
